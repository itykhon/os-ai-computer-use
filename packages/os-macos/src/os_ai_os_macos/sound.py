...
from os_ai_core.utils.logger import get_logger
...
def _resolve_path(path: str) -> str:
    if os.path.isabs(path):
        return path
    # Resolve relative to this package directory to ensure packaged assets are found
    pkg_dir = os.path.dirname(__file__)
    candidate = os.path.join(pkg_dir, path)
    if os.path.isfile(candidate):
        return candidate
    # Fallback: walk up to repo root (developer mode) if needed
